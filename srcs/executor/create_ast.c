...
int mama(int argc, char **argv, char **envp)
{
    t_ast *root = NULL;
    ...
    root = new_node("|", NODE_PIPE);
    root->left = new_node("/usr/bin/ls", NODE_COMMAND);
    root->left->left = new_node("Makefile", NODE_RED_IN);
    root->right = new_node("|", NODE_PIPE);
    root->right->left = new_node("usr/bin/wc", NODE_COMMAND);
    root->right->left->right = new_node("-l", NODE_ARGUMENT);
    root->right->right = new_node("/usr/bin/wc", NODE_COMMAND);
    root->right->right->right = new_node("-l", NODE_ARGUMENT);
    execute_job(root);
    clean_tree(root);
    return 0;
}
